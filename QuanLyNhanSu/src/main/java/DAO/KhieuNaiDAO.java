...
public class KhieuNaiDAO {
	...
	public List<KhieuNai> LayKNChuaDuyet(LoginBean id){
		...
		try{
			...
			while(result.next()) {
				if ("pending".equals(result.getString("TrangThai")) && id.getMaNhanvien().equals(result.getString("MaNgNhan"))) {
	...
	public List<KhieuNai> LayKNDaDuyet(LoginBean id){
		...
		try{
			...
			ResultSet result = pre.executeQuery();			
			while(result.next()) {
				if ("approved".equals(result.getString("TrangThai")) && (id.getMaNhanvien().equals(result.getString("MaNgGui")))) {
					String makn =  result.getString("MaKN");
	...
	public List<KhieuNai> LayKNChoDuyet(LoginBean id){
		...
		try{
			...
			while(result.next()) {
				...
				if ("pending".equals(result.getString("TrangThai")) && (id.getMaNhanvien().equals(result.getString("MaNgGui"))) ) {
					String makn =  result.getString("MaKN");
	...
	private boolean CheckNullDate(Date day) {	
		return day == null;
	}
	private boolean CheckNullContent(String con) {	
		return con == null;
	}
	
	public boolean xoaKhieunai(String id) throws SQLException {
        boolean rowDeleted;
        try (Connection connection = JDBCUtils.getConnection(); PreparedStatement statement = connection.prepareStatement(queryDelete);) {
    ...
    public boolean capnhatKhieunai(String id,String content) throws SQLException {
        boolean rowUpdated;
        try (Connection connection = JDBCUtils.getConnection(); PreparedStatement statement = connection.prepareStatement(queryUpdate);) {
    ...
    public boolean taoKhieunai(String maNggui,String email,String noiDung) throws SQLException {
        ...
        try (Connection connection = JDBCUtils.getConnection(); PreparedStatement statement = connection.prepareStatement(queryCreate);) {
    ...
    public boolean capnhatPH(String id,String content) throws SQLException {
        ...
        try (Connection connection = JDBCUtils.getConnection(); PreparedStatement statement = connection.prepareStatement(queryUpdatePH);) {
            statement.setString(1, content);
